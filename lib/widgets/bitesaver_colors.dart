import 'package:flutter/material.dart';

class BiteSaverColors {
  static const Color pageBackground = Color(0xFFFFFFFF);
  static const Color secondaryBackground = Color(0xFFF6FAFF);
  static const Color surface = Color(0xFFFFFFFF);
  static const Color subtleSurface = Color(0xFFF7FAFC);
  static const Color imageFallback = Color(0xFFEFF6FF);
  static const Color border = Color(0xFFE2E8F0);
  static const Color borderStrong = Color(0xFFCBD5E1);
  static const Color ink = Color(0xFF111827);
  static const Color labelInk = Color(0xFF1F2937);
  static const Color valueInk = Color(0xFF475569);
  static const Color mutedInk = Color(0xFF64748B);
  static const Color softMutedInk = Color(0xFF94A3B8);
  static const Color coolShadow = Color(0xFF0F172A);
  static const Color orange = Color(0xFFD06C3B);
  static const Color orangeDark = Color(0xFFB7542D);
  static const Color green = Color(0xFF5F8F25);
  static const Color greenDark = Color(0xFF4F7D1F);
  static const Color blue = Color(0xFF2563EB);

  const BiteSaverColors._();
}
